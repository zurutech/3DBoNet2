...
public:
  explicit FarthestPointSampleGpuOp(OpKernelConstruction *context)
      : OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("npoint", &npoint_));
    OP_REQUIRES(
        context, npoint_ > 0,
        errors::InvalidArgument("FarthestPointSample expects positive npoint"));
  }
  void Compute(OpKernelContext *context) override {
    ...
    OP_REQUIRES(
        context, inp_tensor.dims() == 3 && inp_tensor.shape().dim_size(2) == 3,
...
REGISTER_KERNEL_BUILDER(Name("FarthestPointSample").Device(DEVICE_GPU),
